...
for row in iter(input_dataset.readline, ''):
    ...
    try:
        if(ignorePoint(float(reading_line[latpick]), float(reading_line[longpick]), float(reading_line[latdrop]), float(reading_line[longdrop]))):
            ...
            index.writerow([(countlines * 2) - 1 - ignoreIndex, countlines, reading_line[latpick], reading_line[
                           longpick], reading_line[pickup_datetime], reading_line[passenger], reading_line[fare], 1])
            index.writerow([(countlines * 2) - ignoreIndex, countlines, reading_line[latdrop], reading_line[
                           longdrop], reading_line[dropoff_datetime], reading_line[passenger], reading_line[fare], 0])
    except:
        ignoreIndex += 2
        print(reading_line[latdrop], reading_line[longdrop],
              reading_line[latdrop], reading_line[longdrop])
...
# indexes
p_index, dataset_line_index, latitude, longitude, hour, passenger, fareu, isPickUp = range(8)
pointValue = 1
aux_pointValue = 1
...
for i in range(countlines):
    ...
    for j in range(pointValue, countlines):
        ...
        similarity = (int(_index_values[passenger]) + int(index_aux_values[passenger])) + (distance * jaccard_similarity([_index_values[fareu], _index_values[passenger], getHour(
            _index_values[hour]), getMinute(_index_values[hour])], [index_aux_values[fareu], index_aux_values[passenger], getHour(index_aux_values[hour]), getMinute(_index_values[hour])]))
